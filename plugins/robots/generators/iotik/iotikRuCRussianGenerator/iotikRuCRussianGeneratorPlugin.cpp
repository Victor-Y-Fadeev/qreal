...
IotikRuCRussianGeneratorPlugin::IotikRuCRussianGeneratorPlugin()
	: IotikRuCGeneratorPluginBase(new robotModel::IotikGeneratorRobotModel(
					"iotikKit"
					, "iotikKitRobot"
					, "IotikRuCRussianGeneratorRobotModel"
					, tr("Generation (RuC) (Russian)")
					, 9 /* After 2D model */)
			, new blocks::IotikBlocksFactory()
			, {":/iotikRuC/templates_russian"})
	, mModel(static_cast<robotModel::IotikGeneratorRobotModel *>(robotModels().first()))
{
}

QString IotikRuCRussianGeneratorPlugin::kitId() const
{
	return "iotikKit";
}
